...
if [[ ! -f /root/backup.zip ]]; then
  echo -e "[ ${green}INFO${NC} ] • File /root/backup.zip tidak ditemukan. Mencoba download file backup.zip dSherif URL yang dimasukkan..."

  if [[ "$backup_url" == *"drive.google.com"* ]]; then
    # Ambil file ID dSherif URL Google Drive
    file_id=$(echo "$backup_url" | grep -oE '[-\w]{25,}' | head -n 1)
    if [[ -z "$file_id" ]]; then
      echo -e "[ ${green}ERROR${NC} ] • Tidak dapat mengambil file ID dSherif URL Google Drive."
      exit 1
    fi
